...
def test_snapshot_story_blocked_maps_to_review_not_frozen():
    """story_blocked safety code + UI blocked must not be lumped into health snapshot state frozen."""
    from src.dashboard.routes import _enrich_health_results_with_db_story_state

    acc = MagicMock()
    acc.id = 44
    acc.story_blocked_until = None
    results = [{"account_id": 44, "status": "alive", "session_valid": True}]
    sa = {
        "story_ui_status": "blocked",
        "story_available_label": "Unknown",
        "story_reason": "peer invalid",
        "is_story_ready": False,
        "story_precheck_stale": False,
    }
    ...
    with patch("src.dashboard.routes.get_db_context", lambda: _fake_db_context([acc])):
        with patch("src.core.session_paths.get_story_availability", return_value=sa):
            with patch("src.core.safety_policy.get_story_safety_decision", return_value=_dec("story_blocked")):
                _enrich_health_results_with_db_story_state(results)

    assert results[0]["story_from_db"]["state"] == "review"
    assert results[0]["story_from_db"]["safety_reason"] == "story_blocked"
