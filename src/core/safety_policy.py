...
def get_story_safety_decision(
    ...
    _BLOCKING_PRECHECK = frozenset({"frozen", "restricted", "rate_limited", "failed_check", "blocked"})
    if precheck in _BLOCKING_PRECHECK:
        reason_map = {
            "frozen": ("Frozen by Telegram.", REASON_STORY_FROZEN),
            "restricted": ("Restricted by Telegram.", REASON_STORY_RESTRICTED),
            "rate_limited": ("Rate limited. Check story_blocked_until.", REASON_STORY_RATE_LIMITED),
            "failed_check": ("Precheck failed. Run story precheck again.", REASON_STORY_PRECHECK_FAILED),
            "blocked": ("Blocked by Telegram.", REASON_STORY_BLOCKED),
        }
    ...
    if precheck != "allowed" and (ss in _BLOCKING_PRECHECK or (sb_dt and sb_dt > now)):
        return StorySafetyDecision(
            allowed=False,
